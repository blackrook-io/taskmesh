...
7. **App version + release notes** — on finish-up, bump SemVer per [.cursor/rules/versioning.mdc](../../rules/versioning.mdc) in the merge commit (MINOR if this Task added a Drizzle migration, otherwise PATCH). In the **same commit**, update repo-root [`RELEASE_NOTES.md`](../../../RELEASE_NOTES.md) (recreate the stub header if the file is missing/empty after a GitHub release clear; prepend newest `## x.y.z — YYYY-MM-DD` with only non-empty Fixes / Enhancements / New Functionality / Breaking Changes / Deprecated Functionality). Mention the new version in the completion comment. Do not skip the bump or the release-notes update.
